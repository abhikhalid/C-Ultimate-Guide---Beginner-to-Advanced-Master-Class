    ...
    class FundsTransfer
    {
        public void Transfer(BankAccount sourceAccount, BankAccount destinationAccount, double amount)
        {
            try
            {
                ...
                {
                    throw new InvalidOperationException($"Insufficient balance in the source account. The current balance is {sourceAccount.CurrentBalance}. But the amount to trasfet is ${amount}");
                }
